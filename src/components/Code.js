...
function Code({children, className, singleline, title, caption, ...other}){
	if (singleline) {
		return (
			<code className={`Code${helper.className("Code--singleline")}${helper.className(className)}`} {...other}>
				{children}
			</code>
		);
	}
...
function CurlyLeft({reserved}){
	return <span className={`${reserved ? "Code__reserved" : "Code__operator"}`}>&#123;</span>;
}
...
function FunctionBlock({statement, name, children, destructured, parameters, arrow}){
	return (
		<>
			{statement ? <Code.Statement>{statement}</Code.Statement> : null}
			{arrow ? null : <Reserved>function</Reserved>}
			<FunctionCall name={name} parameters={parameters} destructured={destructured} />
			{arrow ? <Entity space="both">=<GreaterThan /></Entity> : null}
...
function Variable({children, type="var", name, destructuredArray}){
	return <Line>
		...
		{Array.isArray(name) ? <Code.CommaSeparated items={name} destructured squareBrackets={destructuredArray} /> : <Property>{name}</Property>}
...
function Component({children, tag="DefineTag", props, noSpace, inline}){
	return <Line>
		<LessThan />
		{isComponent(tag) ? <Class>{tag}</Class> : <Reserved noSpace>{tag}</Reserved>}
		{componentProps(props)}
		{children ? null : noSpace ? <Slash /> : <>&nbsp;<Slash /></>}
		<GreaterThan />
		{inline ? children : <Line><Indent>{children}</Indent></Line>}
		{children
			? <>
				<LessThan />
				<Slash />
...
function CommaSeparated({items, destructured, squareBrackets, separator=", ", pretty}){
	...
	var list = items.map((item, index, array) => {
		...
		if (index < array.length - 1) {
			if (typeof itm === "object") {
				return <Fragment key={item + "-" + index}>{itm}<Operator>{separator}</Operator></Fragment>;
			}
			itm = <>{restProp(itm)}<Operator>{separator}</Operator></>;
		}
		if (pretty) {
			return <Line key={item + "-" + index}><Property>{itm}</Property></Line>;
		}
	...
	if (destructured) return squareBrackets ? <><BracketLeft />{list}<BracketRight /></> : <><CurlyLeft />{list}<CurlyRight /></>
	return list;
}
Code.CommaSeparated = CommaSeparated;

function Destructured({children}){
	return <><CurlyLeft />{children}<CurlyRight /></>;
}
Code.Destructured = Destructured;

function String({children}){
	return <span className="Code__string">
		<Quote />
		{children}
		<Quote />
	</span>;
...
function Reserved({children, noSpace}){
	return <><span className="Code__reserved">{children}</span>{noSpace ? null : <>&nbsp;</>}</>;
}
...
function Statement({children, space="after"}){
	return <>
		{space === "after" ? null : <>&nbsp;</>}
		<span className="Code__statement">
			{children}
		</span>
		{space === "before" ? null : <>&nbsp;</>}
	</>;
...
function Indent({level=1, children}){
	return <span className={`Code__indent Code__indent--level-${level}`}>{children}</span>;
}
Code.Indent = Indent;
...
function Comment({children, type="js"}){
	return (
		<span className="Code__comment">
			{type === "js" ? <>&#47;&#47;&nbsp;</> : null}
			{type === "react" ? <><CurlyLeft reserved />&#47;*&nbsp;</> : null}
			{children}
			{type === "react" ? <>&nbsp;*&#47;<CurlyRight reserved /></> : null}
		</span>
	);
}
...
function CssHsl({h="0", s="0%", l="0%"}){
	return (
		<Code.Function.Call name="hsl" parameters={[
...
function CssVar({values}){
	return (
		<Code.Function.Call name="var" parameters={values} />
	);
}
Code.Css.Var = CssVar;
...
function componentProps(props){
	return props?.map(prop =>
		<Fragment key={prop.property}>
			<Attribute property={prop.property}>
				{prop.value}
			</Attribute>
		</Fragment>
	)
