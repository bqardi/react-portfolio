...
function Experience({xp, showLine, isCurrent}){
	...
	return (
		<Card className={`Experience${xp.jobhunting ? " Experience--jobhunting" : ""}`}>
			<div className={`Experience__period${helper.className(`Experience__period--${xp.type.eng}`)}`}>
				<time>
			...
			<Card.Body className={`Experience__body${helper.className(showLine && "Experience__body--line")}`}>
				<div className="Experience__location">
					<div className="Experience__locationBackground">
						<Icon.Philadelphia />
					</div>
					<span className="Experience__locationTitle">{<Translate id="experience-city" />}</span>
					<span className="Experience__locationSeparator">:</span>
					<span className="Experience__locationCity">{translateFeed(xp.location)}</span>
				</div>
				<h3 className="Experience__name">
					{translateFeed(xp.name) || xp.name}
					<span className="Experience__separator">&middot;</span>
					<span className={`Experience__type${helper.className(`Experience__type--${xp.type.eng}`)}`}>
				...
				<p className="Experience__description">
					{parseAttributes(translateFeed(xp.description), {
						link: <Button to="/dashboard/contact">
