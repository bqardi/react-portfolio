...
function DotFade({ amount = 12, dotSizeFactor = 1, animationDuration = 1200 }) {
			...
			arr.push(
				<div
					...
					style={{
						...
						offsetPath: `path("M${size / 2},0 A${size / 2},${size / 2} 0 1,1 ${
							size / 2
						},${size} A${size / 2},${size / 2} 0 1,1 ${size / 2},0")`,
						offsetDistance: `${offsetDistance}%`,
						animationDuration: `${animationDuration / 1000}s`,
						animationDelay: `${
							(animationDuration * offsetDistance) / 100 / 1000
						}s`
					}}
				></div>
			);
...
function Jump({ amount = 3, dotSizeFactor = 1, animationDuration = 2500 }) {
	var { size } = useContext(LoadingContext);
			...
			{[...Array(amount)].map((_, index) => (
				<div
					...
					style={{
						...
						animationDuration: `${animationDuration / 1000}s`,
						animationDelay: `${
							((animationDuration / 4 / amount) * index) / 1000
						}s`
					}}
				></div>
			))}
...
function Rotate({ children, reverse, animationDuration = 2500 }) {
	var { size } = useContext(LoadingContext);
	var offset = size * 0.1;
	return (
		<div
			className="Loading__rotate"
			style={{
		...
		>
			{children || (
				<svg className="Loading__rotateIcon" viewBox="0 0 24 24">
					<path
						className="Loading__rotateIconBack"
						d="M12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4A8,8 0 0,1 20,12A8,8 0 0,1 12,20M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z"
					></path>
					<path
						className="Loading__rotateIconFront"
